	...
	<select name="tag" id="tag">
		...
		<?php 
			...
			while ($array = mysql_fetch_assoc($cloud)){
				do {
					if ($array != NULL) {
						echo "<option value='" . str_replace('&', '&amp;', $array['name']) . "'>" . str_replace('&', '&amp;', $array['name']) . "</option>";
						break;
						} else {
...
while ($books_array = mysql_fetch_assoc($books_selected)){
	if (!empty($books_array)) {
		echo '<table class="bookinfo"><tr><td width="200"><strong>' . $books_array['title'] . '</strong></td><td width="200">';
